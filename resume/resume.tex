...
\resumeSubHeadingListStart
  ...
  \resumeSubheading
    ...
    \resumeItemListStart
      ...
      \resumeItem{
        Designed the \textbf{runtime agent infrastructure} for the Tachyon
        SQA process: a daily regression tracking harness and a durable
        \textbf{test-plan lifecycle} that drafts plans, captures job
        execution details, waits for QA approval, executes jobs
        asynchronously, and writes back job and inspection results.
      }
      \resumeItem{
        Built a \textbf{human-governed self-evolution} loop: the agent
        extracts reusable session memory into a temporary shared-memory
        layer; humans periodically audit and edit it; after approval, the
        agent persists changes into the harness internals.
      }
      \resumeItem{
        Implemented \textbf{Temporal-style durable-execution} primitives:
        deterministic workflow orchestration, activities for side-effecting
        adapters, signals for QA approval, task-queue job claiming,
        idempotent writes, event-history replay, and local sandbox recovery.
      }
    \resumeItemListEnd
  \resumeSubheading
    {Open Ludus}{Remote}
